...
Library of Congress Online Catalog http://catalog.loc.gov (Information on downloading options available at http://catalog.loc.gov/vwebv/ui/en_US/ht
docs/help/faqs.html#14 .)
...
[Taylor, Arlene G. and Joudrey, Daniel N. _The Organization of Information_. 3rd edition, Westport, Conn: Libraries Unlimited. 2009, page 135.]
...
```
_What directory should you clone the files to?_ Make sure that the directory is easy to access from the directory where you will hold your MARC data files that need converting. For example, I placed MARC2CSV in a folder for my programming work, and placed a data folder as a subdirectory within MARC2CSV to hold my MARC data files and the converted .csv files.

If your data file is in a directory in MARC2CSV, your code to convert the file will look like this:

```
$ ../marc2csv.py records.mrc
```
...
```
Depending on where you get your MARC data, it may or may not have the same structure. The sample data came from Library of Congress. I did not know exactly which sub-fields or delimiters were assigned to x1-x5, and so kept the names generic.
